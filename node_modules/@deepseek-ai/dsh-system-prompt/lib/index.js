...
var SystemPrompt = class extends Service {
	...
	*/
	async assemble(context = {}) {
		...
		for (const layer of this.layers.chainLayers(scope)) for (const [name, provider] of layer.variables.entries()) variables[name] = provider(context);
		const sectionByName = this.layers.merge(scope, (layer) => layer.sections);
		const contextByName = this.layers.merge(scope, (layer) => layer.contexts);
		const providers = [...this.layers.global.toolProviders.values(), ...this.layers.chainLayers(scope).flatMap((layer) => [...layer.toolProviders.values()])];
		const collected = [];
		...
		if (completeSection === void 0) return transformed;
		return {
			...transformed,
